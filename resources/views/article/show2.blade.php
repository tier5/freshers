 ...
 function increase_like_article(id)
 {
    ...
    $.ajax({
        ...
        success: function (response){
            $('#like-count').text(response.like);
            $('#dislike-count').text(response.dislike);
            if($('#like-hidden').val() >= response.like){
                $('#article-like-' + id).html('<i class="fa fa-thumbs-o-up"></i>');
            }
            else if($('#like-hidden').val() < response.like){
 ...
 function decrease_like_article(id)
 {
    ...
    $.ajax({
        ...
        success: function (response){
          ...
          $('#like-count').text(response.like);
        $('#dislike-count').text(response.dislike);
            if($('#dislike-hidden').val() >= response.dislike){
                $('#article-dislike-' + id).html('<i class="fa fa-thumbs-o-down"></i>');

            }
            else if($('#dislike-hidden').val() < response.dislike){
 ...
 function increase_like_comment(id)
 {
    ...
    $.ajax({
        ...
        success: function (response){
           ...
           // alert(response);
            $('#like-count-cmmnt-'+id).text(response.like);
            $('#dislike-count-cmmnt'+id).text(response.dislike);

            if($('#comment-like-hidden-'+id).val() >= response.like){
                $('#comment-like-' + id).html('<i class="fa fa-thumbs-o-up"></i>');
            }
            else if($('#comment-like-hidden-'+id).val() < response.like){
    ...
    $.ajax({
        ...
        success: function (response){
          ...
          $('#article_div_dislike_'+id).html(response.dislike);*/
            ...
            if($('#comment-dislike-hidden').val() >= response.dislike){
                $('#comment-dislike-' + id).html('<i class="fa fa-thumbs-o-down"></i>');
            }
            else if($('#comment-dislike-hidden').val() < response.dislike){
        ...
        <p>
            ...
            <div class='col-md-2' id="likes_section_{{$article->id}}">
                ...
                <form action="#" method="POST" id="article_like_form_{{$article->id}}">
                        ...
                        <span id="article_div_like_{{$article->id}}">LIKES: <span id="like-count">{{ $article->likes }}</span> | </span>
                        <input type="hidden" name="like-hidden" id="like-hidden" value="{{ $article->likes }}">
                    </form>
            </div>
               ...
               <div class='col-md-2' id="dislikes_section_{{$article->id}}">
                    <form action="#" method="POST" id="article_dislike_form_{{$article->id}}">
                        ...
                        <span id="article_div_dislike_{{$article->id}}">DISLIKES: <span id="dislike-count">{{ $article->dislikes }}</span> | </span>
                        <input type="hidden" name="dislike-hidden" id="dislike-hidden" value="{{ $article->dislikes }}">
                    </form>
                    ...
                    <div class="col-md-12 social-actions">
                        <div class="col-md-3">
                            <form action="#" method="POST">
                                ...
                                <span id="comment_div_like_{{$comment->id}}">LIKES: <span id="like-count-cmmnt-{{$comment->id}}">{{ $article->likes }}</span> </span> |
                                <input type="hidden" name="comment-like-hidden" id="comment-like-hidden-{{$comment->id}}" value="{{ $article->likes }}">
                            </form>
                        ...
                        <div class="col-md-4">
                            <form action="#" method="POST">
                                ...
                                <span id="comment_div_dislike_{{$comment->id}}">DISLIKES: <span id="dislike-count-cmmnt-{{$comment->id}}">{{ $article->dislikes }}</span> </span> |
                                <input type="hidden" name="comment-dislike-hidden" id="comment-dislike-hidden-{{$comment->id}}" value="{{ $article->likes }}">
                            </form>
                        </div>
